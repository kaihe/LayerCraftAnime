...
def test():
    f_path = r'data\gpt4o_component_description_batch_result\20240812_c435.jsonl'
    ...
    for line in open(f_path).readlines():
        ...
        names.append(_anno['LayerName'])
        details.append(_anno['Explaination'])
        # details.append('\n'.join([k['detail'] for k in _anno['Description']]))
    

    for im, name, desc in zip(ims, names, details):
        # i = _i//col_num
        # j = _i%col_num
        # axarr[i,j].imshow(im)
        ...
        display_image_with_description(im, description)
